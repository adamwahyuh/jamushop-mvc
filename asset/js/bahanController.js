      ...
      bahanList.forEach((item) => {
        ...
        element
          ...
          .addEventListener("submit", (e) => {
            e.preventDefault();
            const bahan_id = element.querySelector(
              "input[name='bahan_id']"
            ).value;
            const porsi = element.querySelector(".porsi-input").value;
            ...
            fetch("http://localhost:9595/api/v1/keranjang", {
              ...
              .then((res) => res.json())
              .then((response) => {
                console.log("Berhasil tambah ke keranjang:", response);
                alert(
                  `Berhasil menambah ${item.nama} ke keranjang!`
                );
              })
              .catch((err) => {
                console.error(err);
